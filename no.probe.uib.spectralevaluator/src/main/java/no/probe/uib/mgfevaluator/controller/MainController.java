...
public class MainController {
    ...
    private Set<DatasetInfo> datasetMap;
...
//            UpdatedDataProcessor updatedDataProcessor1 = new UpdatedDataProcessor();
//            System.out.println("start storing data :-D ");
//            Set<Dataset> dsSet = dsh.loadStoredData();
//            dsSet.stream().map(ds -> {
//                updatedDataProcessor1.reduceDatasetMeasurments(ds);
//                return ds;
//            }).map(ds -> updatedDataProcessor1.prepareDataToTrain(ds)).map(tds -> {
//                updatedDataProcessor1.trainDataset(tds);
//                return tds;
//            }).map(tds -> {
//                dsh.storeDatasetInformation(tds);
//                return tds;
//            }).forEachOrdered(tds -> {
//                System.out.println("at done storing ds " + tds.getDatasetName());
//            });
//        });
//        t.start();
    ...
    private boolean clustrTraningData() {
        return treeCompHandler.clusterTraningData(datasetMap);
    }
    ...
    private boolean importProjects(Set<String> data) {
        ...
        successDownloadFiles.keySet().stream().filter(acc -> (successDownloadFiles.get(acc))).forEachOrdered(acc -> {
            ...
            if (!datastoreHandler.checkDataExisted(projectId)) {
        ...
        projectDatasets.keySet().stream().map(projectId -> {
            ...
            updatedDataProcessor.reduceDatasetMeasurments(projectDataset);
            TraningDataset tds = updatedDataProcessor.prepareDataToTrain(projectDataset);
            updatedDataProcessor.trainDataset(tds);
            datastoreHandler.storeDatasetInformation(tds);

        });
        datasetMap = datastoreHandler.loadDatasetInformation();
...
//            treeCompHandler.compareAllData(datasetMap);
    ...
    private void initDataset(File PSSearchExport, File cmsFile) {
        if (!datastoreHandler.checkDataExisted(PSSearchExport.getName())) {
            ...
            updatedDataProcessor.reduceDatasetMeasurments(dataset);
            TraningDataset tds = updatedDataProcessor.prepareDataToTrain(dataset);
            updatedDataProcessor.trainDataset(tds);
            datastoreHandler.storeDatasetInformation(tds);
            datasetMap = datastoreHandler.loadDatasetInformation();
            MAIN_VIEW_GUI.updateDatasetTable(datasetMap);
